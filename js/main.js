...
//Sélection des éléments (déclarés au début pour éviter les erreurs "undefined")
const burgerMenu = document.querySelector('#burger-menu');
const nav = document.querySelector('nav');

function toggleMenu() {
    if (burgerMenu && nav) { // Vérification défensive
...
let cards = document.querySelectorAll('.card-content'); // Utilisez `let` car 'cards' peut être mis à jour.
...
function afficherCarteSecrete() {
    ...
    if (!carteSecrete) { // si la carte secrete n'existe pas, il faut la créer
        ...
        <img src = "assets/images/carte-flem.png" alt="J'ai la flemme de cuisiner" class="card-image" onerror="this.src='assets/images/logoGourmetech.png'">
        ...
        if (mainCards) { // Vérification défensive
...
//écouteur d'évènement sur la "saisie" dans la barre de recherche LORSQU'une touche est relâchée
if (searchBar) {
    searchBar.addEventListener("keyup", (e) => { // Correction de la syntaxe de l'écouteur
        ...
        if (searchedText === 'flem') { // la détection du code secret "flem" <== EASTER EGG
            ...
            return; // Arrête la fonction pour ne pas filtrer les cartes normales
        }

        // Si ce n'est pas "flem", on re-sélectionne toutes les cartes et on les filtre
        cards = document.querySelectorAll('.card-content:not(.carte-secrete)'); // S'assurer de ne pas inclure la carte secrète si elle existe
        ...
        // Si la carte secrète était visible et que l'utilisateur a effacé "flem" ou tapé autre chose, la cacher
        const carteSecrete = document.querySelector('.carte-secrete');
        if (carteSecrete && searchedText !== 'flem') {
            carteSecrete.style.display = 'none';
        }
    });
}
...
function appliquerFiltres() {
    ...
    // Exclure la carte secrète des filtres classiques
    const currentCards = document.querySelectorAll('.card-content:not(.carte-secrete)');

    // Sélection des valeurs des filtres
    ...
    // Filtrage de cartes
    currentCards.forEach(card => { // ajout de fonction fléchée
        ...
        if (tempsSelectionnes.length > 0 && afficherCarte) {
            ...
            const tempsCorrespond = tempsSelectionnes.some(tempsValue => { // Correction ici
                if (tempsValue === 'rapide' && tempsCard < 30) return true;
                if (tempsValue === 'moyen' && tempsCard >= 30 && tempsCard <= 60) return true;
                if (tempsValue === 'long' && tempsCard > 60) return true;
                return false;
            });
        ...
        if (difficultesSelectionnees.length > 0 && afficherCarte) {
            const difficulteCard = card.dataset.difficulte.toLowerCase(); // Assurez-vous d'être en minuscules pour la comparaison
...
function afficherNotification(message, type) {
    ...
    setTimeout(() => { 
        ...
        setTimeout(() => { 
            notification.remove();
            if (notificationContainer.children.length === 0 && document.getElementById('notification-container')) {
...
function toggleFavorite(event) {
    ...
    if (!card) { 
        console.error("Impossible de trouver la carte parente du bouton like.");
        return;
    }
    const cardId = card.querySelector('h2').textContent.trim(); 
    ...
    if (button.classList.contains('active')) {
        ...
        afficherNotification(`"${cardId}" retiré des favoris.`, 'error'); 
    } else {
        
        button.classList.add('active');
        favorites.push(cardId); 
        afficherNotification(`"${cardId}" ajouté aux favoris ! ❤️`, 'success'); 
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const formContact = document.querySelector('.contact-form form'); 
    if (formContact) {
...
function displayFavoriteCards() {
    ...
    favoriteIds.forEach(id => {
        ...
        if (recipe) {
            ...
            favoriteCardElement.setAttribute('data-temps', recipe.tempsNum); 
            favoriteCardElement.setAttribute('data-difficulte', recipe.difficulte.toLowerCase()); 
            ...
            console.warn(`Détails de recette introuvables pour l'ID: ${id}. Cette recette a peut-être été supprimée ou son ID a changé. Elle sera retirée de vos favoris.`);
           
        }
    });


    if (updatedFavorites.length !== favoriteIds.length) {
